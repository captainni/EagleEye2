...
| 表名 | 说明 | 关键字段 |
|------|------|----------|
| `policy_info` | 政策信息表 | id, title, source, source_url, publish_time, content, policy_type, importance, areas |
| `policy_analysis` | 政策分析表 | policy_id, summary, key_points, impact_analysis |
...
| 表名 | 说明 | 关键字段 |
|------|------|----------|
| `competitor_info` | 竞品信息表 | id, title, company, type, capture_time, tags, content, sources, summary |
| `competitor_analysis` | 竞品分析建议表 | competitor_id, content, sort_order |
...
| 表名 | 说明 | 关键字段 |
|------|------|----------|
| `crawler_config` | 爬虫配置表 | config_id, target_name, target_type, source_urls, crawl_depth, trigger_schedule, extraction_strategy_type |
| `crawler_task_log` | 爬虫任务执行日志表 | log_id, task_id, config_id, target_url, start_time, end_time, status, error_message |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/v1/user/subscriptions` | 获取用户订阅列表 |
| POST | `/v1/user/subscriptions` | 创建订阅 |
| PUT | `/v1/user/subscriptions/{subscriptionId}` | 更新订阅 |
| DELETE | `/v1/user/subscriptions/{subscriptionId}` | 取消订阅 |

#### 4.3.7 系统设置模块

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/v1/settings` | 获取用户设置 |
| PUT | `/v1/settings` | 更新用户设置 |
...
项目使用 Pinia 进行状态管理，主要 Store：

| Store | 状态 |
|-------|------|
| `userStore` | 用户登录信息、权限 |
| `policyStore` | 政策筛选条件、缓存 |
| `competitorStore` | 竞品筛选条件、缓存 |
| `requirementStore` | 需求列表状态 |
| `settingsStore` | 用户设置 |
